...
halfsp_dgp <- function(N, X) {
  ...
  und_lin = -0.2 + 0.8*X[, 1] - 0.1*X[, 2] + 0.2*X[, 8] + 0.5*X[, 9]*X[, 10] + runif(N)/10
  pscore <- pnorm( und_lin )
  
  Z = rbinom(N, 1, pscore)
  
  ####### RESPONSE SURFACES
  mu = 3 + 0.5*(X[, 3] - 0.5)^2 + 1.5*(2-abs(X[, 4])) + 1.5*(X[, 6] + 1)
  ITE = 0.2 + 1*abs(X[, 5] - 1) - 0.4*X[, 7]
...
normsp_dgp <- function(N, X) {
  ...
  ####### RESPONSE SURFACES
  mu = 3 + 0.5*(X[, 1] - 0.5)^2 + 1.5*(2-abs(X[, 2])) + 1.5*(X[, 8] + 1)
  ITE = 0.2 + 1*abs(X[, 9] - 1) - 0.4*X[, 10]
